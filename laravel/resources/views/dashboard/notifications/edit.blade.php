                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <span class="text-secondary font-italic">(مثال توضيحي) {{ getClassData($notification->body['ar']) }}</span>
                        ...
                        <span class="text-secondary font-italic"> {{ getClassData($notification->body['en']) }} (for example)</span>
                                        ...
                                        @if (isset($model['additional']))
                                            <div class="additional-checkboxes card p-1 mt-2"
                                                ...
                                                <div class="card-header bg-primary text-white">
                                                    {{ removeAppModels($model['className']) }} Additional Options (نوع
                                                    المستلم)
                                                </div>
                                                <div class="card-body">
                                                    @foreach ($model['additional'] as $key => $value)
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox"
                                                                name="to_model_additional[]" value="{{ $value }}"
                                                                id="additional-{{ $key }}"
                                                                {{ in_array($value, $model_additional['to_model_additional']) ? 'checked' : '' }}>
                                            ...
                                            <div class="additional-checkboxes card p-1 mt-2"
                                                ...
                                                <div class="card-body">
                                                    @foreach ($model['additional'] as $key => $value)
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox"
                                                                name="for_model_additional[]" value="{{ $value }}"
                                                                id="for-additional-{{ $key }}"
                                                                {{ in_array($value, $model_additional['for_model_additional']) ? 'checked' : '' }}>
                                                            <label class="form-check-label"
                                                                for="for-additional-{{ $key }}">
                                                                {{ $value }}
                                                            </label>
                                                        </div>
                                                    @endforeach
...
@push('page_scripts_vendors')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            // استدعاء الدالة لتحديث الحقول الإضافية بناءً على القيمة الحالية
            const toSelectElement = document.getElementById('to_model_type');
            const forSelectElement = document.getElementById('for_model_type');

            // تحقق من أن العناصر موجودة، ثم استدعي الدالة
            if (toSelectElement) {
                updateSelectedModelIndex('to', toSelectElement);
            }
            if (forSelectElement) {
                updateSelectedModelIndex('for', forSelectElement);
            }
        });
        ...
        //             additionalCheckboxes.style.display = 'block'; // Show the relevant additional checkboxes
        //         }
        //     }

        //     // Ensure the visibility of the additional div
        //     additionalDiv.style.display = additionalDiv.querySelector('.additional-checkboxes[style*="display: block"]') ? 'block' : 'none';
        // }
        function updateSelectedModelIndex(type, selectElement) {
            const models = @json($models); // جلب النماذج من الخادم
            const selectedIndex = selectElement.selectedIndex - 1; // تعدل لحساب الخيار الفارغ الأول
            const selectedModel = models[selectedIndex]; // احصل على النموذج المختار
            const additionalDiv = document.getElementById(type + '_additional');
            ...
            if (selectedModel) {
                // إظهار الخصائص الإضافية المرتبطة بالنموذج المختار
                const additionalCheckboxes = additionalDiv.querySelector(
                    `[data-model="${selectedModel['className'].replace(/\\/g, '')}"]`);

                if (additionalCheckboxes) {
                    additionalCheckboxes.style.display = 'block'; // إظهار checkboxes الإضافية
                }
            }

            // تأكد من إظهار div إذا كان هناك checkboxes مرئي
            const hasVisibleCheckboxes = !!additionalDiv.querySelector('.additional-checkboxes[style*="display: block"]');
            additionalDiv.style.display = hasVisibleCheckboxes ? 'block' : 'none';
        }
    </script>
@endpush
